...
- **Расстояние между секциями: 100px** (утверждено v30)
...
- **Расстояние между секциями: 60px**
- **Мобильные значения НЕ удваивать** — они корректируются отдельно

### Правило межсекционных отступов (КРИТИЧНО, v30)
...
>
> **Текущая схема:**
> - Hero: `padding: 100px 144px 0` (десктоп), `padding: 30px 20px 0` (мобайл) — bottom = 0
> - Quote: `padding: 100px 0` (десктоп), `padding: 60px 0` (мобайл) — обеспечивает gap сверху и снизу
> - Steps: `padding: 0 0 120px` (десктоп), `padding: 0 0 60px` (мобайл) — top = 0
>
> **При добавлении новой секции:** проверить стык с соседними. Если фон одинаковый — top padding = 0, расстояние обеспечивает предыдущая секция.
>
...
- border-radius: 4px
- z-index: -1
- **ВАЖНО (stacking context):** `::before` с `z-index: -1` работает только если у ближайшего parent-контейнера заголовка есть `position: relative; z-index: 0`. Без этого подложка уходит за фон секции. В Hero это работает автоматически, в ДРУГИХ секциях — добавить `position: relative; z-index: 0` на контейнер заголовка.
...
## Карточки (ОБЯЗАТЕЛЬНЫЕ СТАНДАРТЫ ПО ТИПАМ)

> **КРИТИЧНО:** Каждый тип карточек имеет ФИКСИРОВАННЫЕ параметры. НЕ отступать от стандартов без явного согласования.

### Тип 1: Контентные карточки с изображениями (утверждено на steps-section v30, quote-test)
**Применение:** карточки бонусов (steps), видео-карточки, медиа-карточки

**Геометрия (ЖЕЛЕЗНЫЕ ПАРАМЕТРЫ):**
- **Border-radius карточки:** 16px — ВСЕГДА
- **Border-radius изображений/контента внутри:** 12px — ВСЕГДА
- **Padding базовый:** 12px (белая рамка вокруг контента)
- **Gap между элементами внутри:** 12px

**Стиль:**
- Фон: #ffffff (белые), #c8f542 (акцентные/лаймовые), #fafafa (нейтральные)
- Тень: 0 25px 60px rgba(0,0,0,0.10), 0 6px 20px rgba(0,0,0,0.05)
- Hover: translateY(-6px), усиление тени

**Примеры использования:**
- steps-section: карточки бонусов с изображениями
- quote-test: салатовая карточка с видео 720×405px

### Другие типы карточек
*Будут добавлены по мере утверждения новых экранов*
